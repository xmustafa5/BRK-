...
export default function SingleDistributors({ item, ar }) {
  return (
    <div className="mt-4 bg-white shadow-[0px_20px_30px_0px_rgba(0,0,0,0.10)]">
      ...
      <div className="flex border-t mt-3 border-gray-500 justify-center items-start p-2 gap-4">
        <div className="flex text-md justify-center  items-start flex-col">
          <p>
            {ar ? "الهاتف:" : "phone:"}{" "}
            <span className="text-gray-120 ml-7">{item?.phone}</span>
          </p>
          <p>
            {ar ? "الموقع:" : "Website:"}{" "}
            <span className="text-gray-120 ml-4">{item?.website}</span>
          </p>
        </div>
        <div className="max-w-[270px] flex">
          <p>{ar ? "العنوان:" : "Address:"} </p>
          <p className="text-gray-120 ml-4 min-w-[10px]">{item?.address}</p>
        </div>
      </div>
    </div>
  );
